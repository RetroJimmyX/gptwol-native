  ...
  with open(cron_filename) as f:
    for line in f:
      ...
      if len(fields) >= 7:
        ...
        command = fields[6:]
        if len(command) >= 2:
          schedules.append((schedule, command[-1]))
  return schedules
...
def load_computers():
  ...
  for schedule, mac_address in parse_schedule_file():
...
def check_invalid_mac(mac):
  ...
  return re.match(mac_pattern, mac) is None
...
@login_required
def delete_wol_cron():
  return delete_cron_entry(request.form['mac_address'])
...
def delete_sol_cron():
  request_mac_address = request.form['mac_address']
  reversed_mac_address = ':'.join(reversed(request_mac_address.split(':')))
  return delete_cron_entry(reversed_mac_address)
...
def check_status():
  ...
  return 'awake' if is_computer_awake(ip_address, test_type) else 'asleep'
...
def wol_or_sol_send():
  ...
  messages = [f"Wake On Lan Magic Packet Sent to {computer['name']}!"]
  return generate_modal_html(messages, 'Wakeup')
