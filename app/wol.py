...
# Native/container-neutral paths. Set GPTWOL_DATA_DIR, GPTWOL_CRON_FILE and
# GPTWOL_WAKEONLAN_COMMAND when the defaults do not suit the installation.
data_dir = os.environ.get('GPTWOL_DATA_DIR', os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'db'))
cron_filename = os.environ.get('GPTWOL_CRON_FILE', os.path.join(data_dir, 'gptwol.cron'))
wakeonlan_command = os.environ.get('GPTWOL_WAKEONLAN_COMMAND', 'wakeonlan')
computer_filename = os.path.join(data_dir, 'computers.txt')
...
db_path = os.path.join(data_dir, 'computers.db')
...
def is_computer_awake(ip_address, port):
  ...
  return is_computer_awake_tcp(ip_address, int(port))
...
def check_invalid_mac(mac):
  return not bool(re.match(r'^([0-9a-fA-F]{2}:){5}([0-9a-fA-F]{2})$', mac))
...
def add_computer():
  ...
  messages = []
  if check_mac_exist(mac_address): messages.append(f'Computer mac: {mac_address} already exists.')
  if check_invalid_name(name): messages.append(f'NAME: {name} is invalid. Character , is invalid')
  if check_invalid_ip(ip_address): messages.append(f'IP: {ip_address} is invalid.')
  if check_invalid_mac(mac_address): messages.append(f'MAC: {mac_address} is invalid.')
  if check_invalid_test_type(test_type): messages.append(f'Status check: {test_type} is invalid. Enter "icmp", "arp" or a valid TCP port number.')
  if messages: return generate_modal_html(messages, 'Add Computer Error')
  db.session.add(Computer(name=name, mac_address=mac_address, ip_address=ip_address, test_type=test_type))
  db.session.commit()
  return redirect(url_for('wol_form'))
...
def edit_computer():
  ...
  messages = []
  if computer_to_edit is None: messages.append(f'Computer with MAC address: {mac_address} not found.')
  if check_invalid_name(name): messages.append(f'NAME: {name} is invalid. Character , is invalid')
  if check_invalid_ip(ip_address): messages.append(f'IP: {ip_address} is invalid.')
  if check_invalid_test_type(test_type): messages.append(f'Status check: {test_type} is invalid. Enter "icmp", "arp" or a valid TCP port number.')
  if messages: return generate_modal_html(messages, 'Edit Computer Error')
  if computer_to_edit.name == name and computer_to_edit.ip_address == ip_address and computer_to_edit.test_type == test_type:
    return generate_modal_html(['No change was made.'], 'Edit Computer Info')
...
def add_cron(mac_address, request_cron):
  ...
  cron_command = f"{request_cron} root {wakeonlan_command} {mac_address}"
...
@login_required
def add_wol_cron():
  return add_cron(request.form['mac_address'], request.form['cron_request'])
...
def add_sol_cron():
  ...
  return add_cron(reversed_mac_address, request.form['cron_request'])

@app.route('/delete_wol_cron', methods=['POST'])
@login_required
def delete_wol_cron():
  delete_cron_entry(request.form['mac_address'])
  return redirect(url_for('wol_form'))
...
@login_required
def check_status():
  if is_computer_awake(request.args.get('ip_address'), request.args.get('test_type')):
    return 'awake'
  return 'asleep'
...
def wol_or_sol_send():
  ...
  if is_computer_awake(ip_address, test_type):
    ...
    if l2_wol_packet:
      messages.append('Wake On Lan Mode: L2 Packet')
      send_l2_wol_packet(mac_address, l2_interface)
    else:
      messages.append('Wake On Lan Mode: L4 Packet')
